...
namespace Lemonade.Action.GeneralContext
{
    /// <summary>
    /// 事件动作上下文
    /// </summary>
    public class EventContext  : IActionContext
    {
        ...
        public void Activate()
        {
            ...
            //{
            //    //int sCount = this.actions[mc].Count;
            //    List<IAction> tmpList = this.actions[mc].ToList();
            //    //for (int i = 0; i < tmpList.Count; i++)
            //    //{
            //    //    this.RemoveAction(this.actions[mc][i]);
            //    //}
            //    foreach (IAction ac in tmpList)
            //    {
        ...
        public void AddAction(IAction Action)
        {
            if (Action != null)
            {
                if (this.ContextProcessType == Lemon.GetObjType(Action.InvokeObject))
                {
                    ...
                    Lemon.GetObjType(Action.InvokeObject).GetEvent(Action.ActionName).AddEventHandler(Action.InvokeObject, Action.InvokeProcessor);
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual IAction FindAction(IAction TargetAction)
        {
            IAction result = null; 
            //获取当前调用者关键字类型的动作列表
            var ik = this.actions.Keys.ToList().Find(delegate(object mc) { return mc == TargetAction.InvokeObject; });
            if (ik != null)
            {
                //查找匹配的动作
                result = this.actions[ik].ToList().Find(delegate(IAction ac) 
                { return (ac.ActionName == TargetAction.ActionName 
                    && ac.Type == TargetAction.Type
                    && ac.InvokeProcessor.Method == TargetAction.InvokeProcessor.Method);
                }); 
            } 
            return result;
        ...
        public void RemoveAction(IAction Action)
        {
            if (Action != null)
            {
                if (this.ContextProcessType == Lemon.GetObjType(Action.InvokeObject))
                {
                    IAction rAction = this.FindAction(Action);
                    if (rAction != null)
                    {
                        if (rAction.Performer != null)
                        {
                            rAction.Performer(rAction.InvokeObject);
                        }
                        this.actions[Action.InvokeObject].Remove(rAction);
                        Lemon.GetObjType(rAction.InvokeObject).GetEvent(rAction.ActionName).RemoveEventHandler(rAction.InvokeObject, rAction.InvokeProcessor);
                    }
                    //var ik = this.actions.Keys.ToList().Find(delegate(object mc) { return mc == Action.InvokeObject; });
                    //if (ik != null)
                    //{
